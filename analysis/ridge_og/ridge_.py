...
import numpy as np
import logging
ridge_og_dir = '/Users/rajviagravat/Documents/Hamilton_Lab/Code/speechMusic/analysis/'
sys.path.append(ridge_og_dir)
import ridge_og.utils
from ridge_og.utils import mult_diag, counter
import random
...
def bootstrap_ridge(Rstim, Rresp, Pstim, Presp, alphas, nboots, chunklen, nchunks,
    ...
    for bi in counter(range(nboots), countevery=1, total=nboots):
        ...
        if use_svd:
            ...
            Rcmat = eigridge_corr(RRstim, PRstim, RRresp, PRresp, alphas,
                           corrmin=corrmin, singcutoff=singcutoff,
                           normalpha=normalpha, use_corr=use_corr,
                           logger=logger, force_cmode=False)
    ...
    if return_wts:    
        ...
        if use_svd:
            ...
            wt = eigridge(Rstim, Rresp, valphas, singcutoff=singcutoff, normalpha=normalpha, force_cmode=False)
...
def bootstrap_ridge_shuffle(orig_STRF, Rstim, Rresp, Pstim, Presp, valpha, nboots, chunklen,
    ...
    for bi in counter(range(nboots), countevery=1, total=nboots):
        ...
        if use_svd:
            ...
            wt = eigridge(Rstim, RRresp, valpha, singcutoff=singcutoff, normalpha=normalpha, Q=Q, L=L, covmat=covmat, force_cmode=False)
