...
def make_envelopes(wav_dir, wav_name, new_fs, ep, pad_next_pow2=True):    
	...
	envelopes = []
	if sound.ndim > 1:
		sound=sound[:,1] #take only the first track 
		print(sound.shape)	
...
def stimuli_mel_spec(path, wav_name, new_fs):
	...
	w=w.astype(float)
	if w.ndim > 1:
		w=w[:,1] #take only the first track 
		print(w.shape)
...
def get_bin_edges_percent_range(a, bins=20, percent=95):
    assert percent > 1 
    assert percent < 100
    tail_percentage = (100 - percent) / 2
    
    #inf or NAN - add a line to zero these out
    a = np.nan_to_num(a, nan=0.0, posinf=0.0, neginf=0.0)

    # Get the percentiles to define the range
    min_val, max_val = np.percentile(a, [tail_percentage, 100 - tail_percentage])

    # Generate logarithmically spaced bin edges
    bin_edges = np.logspace(np.log2(min_val), np.log2(max_val), num=bins, base=2)
    
    # Plot histogram
    plt.hist(a, bins=bin_edges, edgecolor='black')
    plt.xscale('log')  # Set x-axis to logarithmic scale for better visualization
    plt.xlabel('Value')
    plt.ylabel('Frequency')
    plt.title('Histogram of Data with Logarithmic Bins')
    plt.show()

    return bin_edges
    #histogram - for bin edges ipyt
...
def trailer_soundCat_matrix(nat_sound_event_dir, wav_name, evs, epochs, show_fig=False):
	...
	# name = wav_name.removesuffix('.wav') + '_corrected_natsounds.txt'
	# Python 3.8 compatible version
	if wav_name.endswith('.wav'):
	    name = wav_name[:-4] + '_corrected_natsounds.txt'
	else:
	    name = wav_name + '_corrected_natsounds.txt'
