...
def plot_strf_direct(data_dir, subject, save_dir, strf_type, subplot_r, subplot_c,
    ...
    output_dir = f'/{data_dir}/sub-{subject}/strfs_og/plot_figs'
    ...
    with h5py.File('%s/sub-%s/strfs_og/STRF_by_%s_MT.hf5'%(data_dir, subject, strf_filename),'r') as hf:
        wts = hf['/wts'][:]
        corrs = hf['/corrs'][:]
    ...
    if plot_all_chs:
        ...
        plt.tight_layout()
        plt.savefig('%s/sub-%s/strfs_og/plot_figs/%s_wts_STRF_subplots.pdf' %(save_dir, subject, strf_type))
...
def plot_strfs_for_multiple_subjects(subject_list, fs=100.0, delay_min=0.0, delay_max=1.0):
    ...
    for subject in subject_list:  # Iterate through the subject list
        ...
        try:
            ...
            print(f"Successfully plotted for {subject}")
        except Exception as e:
            print(f"Error plotting for {subject}: {str(e)}")
...
# Example usage
# Plot STRFs for all subjects in the list 
